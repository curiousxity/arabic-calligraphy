...
test.describe("square kufi", () => {
  ...
  test("a mirror draws its source's hand-painted cells", async ({ page }) => {
    // `MirrorBlockView` resolves the source and mounts the source's *own*
    // renderer, so a prop it forgets to forward is silently absent rather than
    // a type error — which is exactly how `strokeCuts` went missing there
    // once already.
    //
    // The observable is the two boxes agreeing. A cell painted clear of the
    // panel grows the grid, so the source's box widens; a mirror that never
    // received the edits keeps drawing the ungrown grid and stays narrower by
    // the whole reach of that cell.
    await gotoApp(page);
    const id = await addKufi(page);
    await setBlockText(page, "محمد", id);

    const { origin, cellPx } = await armCellPainter(page, id);
    const target = await cellPoint(page, origin, cellPx, -3, -3);
    ...
    await page.locator('button[aria-label="Reset view"]').click();
    // The mirror measures its own drag surface in a settle loop, so wait for
    // that box to stop being the small fallback before reading it.
    await expect
      .poll(async () => (await blockClientBox(page, mirror.id)).width)
      .toBeGreaterThan(20);
